...
- the operator wallet holds only the native balance needed for staging;
...
- per-transfer sUSDC ceiling and fixed native-gas amount;
- per-recipient request limit;
- global sUSDC and native-gas budgets per configured window;
...
currently authorizes both sUSDC and native-gas operations.
...
transfers. The same key can directly transfer its entire native balance.

For staging, this is accepted because the token is test-only and the native
balance must remain small. Before assets have value, add a cumulative on-chain
...
native balance. A configured maximum gas price is a deferred control. A
...
- inspect and minimize the operator's native balance;
